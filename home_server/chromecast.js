...
Chromecast.prototype.getVolume = function(address) {
  var self = this;
  return new Promise(resolve => {
    var castv2Client = new Castv2Client();
    castv2Client.connect(self.parseAddress(address), function() {
      castv2Client.getVolume((r) => {
        resolve(r);
      });
    });
    castv2Client.on('error', function(err) {
...
Chromecast.prototype.stop = function(address) {
  var self = this;
  return new Promise(resolve => {
    var castv2Client = new Castv2Client();
    castv2Client.connect(self.parseAddress(address), function() {
      castv2Client.launch(DefaultMediaReceiver, function(err, player) {
        castv2Client.stop(player, () => {
          resolve(null);
        });
      });
    });
